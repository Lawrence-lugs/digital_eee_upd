---
title: "Module 111: Best Practices and Wrap-Up"
format:
  html:
    css: vcc-v2.css
---
...
![concurrency](./Course_ Verilog Crash Course_files/concurrency(1).png){width=40%}
...
![timing_sync](./Course_ Verilog Crash Course_files/timing_sync.png){width=70%}
...
![timing_async](./Course_ Verilog Crash Course_files/timing_async.png){width=70%}
...
![](./Course_ Verilog Crash Course_files/combinational_loop.png){width=45%}
...
![sequential_loop](./Course_ Verilog Crash Course_files/sequential_loop.png){width=55%}

---

## 7) Avoid inferred latches in combinational logic
...
![latch](./Course_ Verilog Crash Course_files/latch.png){width=40%}
...
![latch_soln](./Course_ Verilog Crash Course_files/latch_soln.png){width=35%}
...
![latch_soln_default](./Course_ Verilog Crash Course_files/latch_soln_default(1).png){width=35%}
...
![latch_seq](./Course_ Verilog Crash Course_files/dff_enable.png){width=45%}

---

## 8) One driver per signal (no double drivers)
...
![double_driver_ex1](./Course_ Verilog Crash Course_files/double_driver_ex1.png){width=50%}
...
![](./Course_ Verilog Crash Course_files/double_driver_soln(1).png){width=55%}

---

::: {.callout-note}
Always think about **mutual exclusivity**. **All assignments to the same signal must always be mutually exclusive.** At any given time, no two assignments to the same signal must take place. Use `else` to ensure mutual exclusivity.
:::

## 9) Keep non-synthesizable constructs in the testbench
...
## 10) Prefer `localparam` for states and special constants
...
## 11) Practical timing tip (when you see “one-cycle late” flags)
...
- **000 — Basic Structure and Syntax**
    You learned the shape of a Verilog design: `module … endmodule`, ports, vectors,
...
- **001 — Writing Combinational Logic**
    You described logic with `assign`, sized constants (`'b`,
...
- **010 — Advanced Combinational Logic**
    You shifted to `always @(*)` for richer logic, expressed choices with `if/else` and `case`, and learned two pitfalls to avoid: **double drivers** and **inferred latches** in combinational code (cover all branches or give defaults).
- **011 — Writing Sequential Logic**
    You moved time into the picture with `always @(posedge clk)`. The **golden rule**: use `<=` in clocked blocks (and `=` in `@(*)`).
...
- **100 — Advanced Sequential Logic**
    Algorithms → **FSMs**: states as steps, one step per clock (the clocked block
...
- **101 — Testbench Basics and Verification**
    You wrapped designs in a simulation harness: UUT instantiation, clock generation, `initial` blocks, delays, `$display/$finish`, and wave dumps—**testbench-only constructs** that help you see and debug behavior.
- **110 — Advanced Verification Ideas**
    You leveled up to **self-checking**: reusable `task`-based
...
- **111 — Best Coding Practices (here)**
    You collected guardrails for rapid, reliable prototyping: clear names, the golden assignment rule, reset priority, no
...
::: {.vcc-nextprev}
[← M110](110-advanced-verification.qmd){.vcc-prev} [Extras →](05-extras.qmd){.vcc-next}
:::
